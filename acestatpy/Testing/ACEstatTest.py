...
class ACEstatTest(object):
  ...
  def generateFilename(self):
    return sanitize_filename("{0}_{1}.csv".format(datetime.fromtimestamp(self.__startTime).strftime(
        '%Y%m%d-%H%M%S'), self.info.name.replace(' ', '-')))

  def export(self, path=None):
    dname = self.generateFilename()
    if not path:
...
class TestParameters(object):
  def __init__(self, id, parameters=None):
    ...
    for item in Tests.get(id).parameters.items():
      if item[1].type == "static":
        parameters[item[0]] = item[1].value
        continue
      elif not parameters.get(item[0], False):
        raise ParameterError(id, f"Missing parameter {item[0]}")
      ...
      elif item[1].type == "float":
        try:
          val = float(parameters.get(item[0]))
        except Exception as e:
          raise ParameterError(item[0], "Expected float")
      elif item[1].type == "number":
        try:
          val = float(parameters.get(item[0]))
          if ((item[1].float_min and val < item[1].float_min) or
                  (item[1].float_max and val > item[1].float_max)):
            parameters[item[0]] = f"{int(val)}"
            val = float(int(val))
        except Exception as e:
          raise ParameterError(item[0], "Expected number")
  ...
  def getFormatted(self, key):
    ...
    if param.type == "float":
      val = f"{abs(float(val)):.{param.precision}f}"
      val, dec = val.split(".")
      dec = "." + dec
    elif param.type == "int":
      val = str(abs(int(val)))
    if hasattr(param, "lpad"):
      width = param.lpad.width
      val = val.rjust(width, param.lpad.char)
    if hasattr(param, "rpad"):
      width = parame.rpad.width
      val = val.ljust(width, param.rpad.char)
    if getattr(param, "signed", False):
      if negative:
        val = f"-{val}"
      else:
